...
function VoiceOver.ReportError(where, err)
    local msg = "|cffff5555VoiceOver [" .. tostring(where) .. "]: " .. tostring(err) .. "|r"
    if DEFAULT_CHAT_FRAME then
        DEFAULT_CHAT_FRAME:AddMessage(msg)
        if debugstack then
            DEFAULT_CHAT_FRAME:AddMessage("|cffffaaaa" .. debugstack(2, 12, 2) .. "|r")
        end
    end
end
