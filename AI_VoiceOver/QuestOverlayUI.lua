...
function QuestOverlayUI:CreatePlayButton(questID)
    ...
    local disabled = playButton.GetDisabledTexture and playButton:GetDisabledTexture()
    if disabled then
        if disabled.SetDesaturated then
            disabled:SetDesaturated(true)
        end
        disabled:SetAlpha(0.33)
    end
...
function QuestOverlayUI:UpdatePlayButton(soundTitle, questID, questLogTitleFrame, normalText, questCheck)
    ...
    self.questPlayButtons[questID]:SetScript("OnClick", function(clicked)
        clicked = clicked or this
        if not QuestOverlayUI.questPlayButtons[questID].soundData then
            local qtype, id = DataModules:GetQuestLogQuestGiverTypeAndID(questID)
            QuestOverlayUI.questPlayButtons[questID].soundData = {
                event = Enums.SoundEvent.QuestAccept,
                questID = questID,
                name = id and DataModules:GetObjectName(qtype, id) or "Unknown Name",
                title = soundTitle,
                unitGUID = id and Enums.GUID:CanHaveID(qtype) and Utils:MakeGUID(qtype, id) or nil
            }
        end

        local soundData = clicked and clicked.soundData or QuestOverlayUI.questPlayButtons[questID].soundData
        ...
        if not isPlaying then
            ...
            soundData.stopCallback = function()
                QuestOverlayUI:UpdatePlayButtonTexture(questID)
                if clicked then
                    clicked.soundData = nil
                end
                QuestOverlayUI.questPlayButtons[questID].soundData = nil
            end
        else
...
function QuestOverlayUI:Update()
    ...
    for i = 1, (QUESTS_DISPLAYED or 6) do
        ...
        if not questLogTitleFrame or not normalText then
            break
        end
