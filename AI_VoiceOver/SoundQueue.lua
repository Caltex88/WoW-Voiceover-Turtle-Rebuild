...
function SoundQueue:AddSoundToQueue(soundData)
    local ok, err = pcall(SoundQueue._AddSoundToQueue, self, soundData)
    if not ok then
        ReportError("AddSoundToQueue", err)
    end
end
function SoundQueue:_AddSoundToQueue(soundData)
    if not DataModules:PrepareSound(soundData) then
        if soundData.questID then
            DEFAULT_CHAT_FRAME:AddMessage("|cffffaa00VoiceOver: no sound file for " .. tostring(soundData.title or soundData.name or "?") .. " (quest " .. tostring(soundData.questID) .. ")|r")
        end
        return
    end

    if not Utils:IsSoundEnabled() then
        DEFAULT_CHAT_FRAME:AddMessage("|cffffaa00VoiceOver: game sound is off (MasterSoundEffects / Sound_EnableSFX)|r")
        return
    end
