...
function SoundQueueUI:InitDisplay()
    ...
    self.frame.resizer:HookScript("OnMouseDown", function()
        local highlight = self.frame.resizer.GetHighlightTexture and self.frame.resizer:GetHighlightTexture()
        if highlight then
            highlight:Hide()
        end
    ...
    function self.frame.container.name:Update()
        local containerWidth = (self:GetParent():GetRight() or 0) - (self:GetParent():GetLeft() or 0)
        self:SetWidth(0)
    ...
    local stopHighlight = self.frame.container.stopGossip.GetHighlightTexture and self.frame.container.stopGossip:GetHighlightTexture()
    if stopHighlight then
        stopHighlight:SetAlpha(0.5)
    end
    local stopPushed = self.frame.container.stopGossip.GetPushedTexture and self.frame.container.stopGossip:GetPushedTexture()
    if stopPushed then
        stopPushed:SetAlpha(0.5)
    end
...
function SoundQueueUI:InitPortraitLine()
    ...
    local miniNormal = self.frame.miniPause:GetNormalTexture()
    if miniNormal then
        miniNormal:ClearAllPoints()
        miniNormal:SetPoint("CENTER")
        miniNormal:SetSize(14, 14)
    end
    self.frame.miniPause:SetPushedTexture([[Interface\AddOns\AI_VoiceOver\Textures\PortraitFrameAtlas]])
    local miniPushed = self.frame.miniPause:GetPushedTexture()
    if miniPushed then
        miniPushed:ClearAllPoints()
        miniPushed:SetPoint("CENTER")
        miniPushed:SetSize(12, 12)
    end
    ...
    function self.frame.miniPause:Update()
        ...
        if not normal or not pushed then
            return
        end
        if Addon.db.char.IsPaused then
            normal:SetTexCoord(0 / PORTRAIT_ATLAS_SIZE, 93 / PORTRAIT_ATLAS_SIZE, 419 / PORTRAIT_ATLAS_SIZE, 512 / PORTRAIT_ATLAS_SIZE)
            pushed:SetTexCoord(0 / PORTRAIT_ATLAS_SIZE, 93 / PORTRAIT_ATLAS_SIZE, 419 / PORTRAIT_ATLAS_SIZE, 512 / PORTRAIT_ATLAS_SIZE)
            normal:SetAlpha(MouseIsOver(self) and 1 or 0.75)
        else
            normal:SetTexCoord(93 / PORTRAIT_ATLAS_SIZE, 186 / PORTRAIT_ATLAS_SIZE, 419 / PORTRAIT_ATLAS_SIZE, 512 / PORTRAIT_ATLAS_SIZE)
            pushed:SetTexCoord(93 / PORTRAIT_ATLAS_SIZE, 186 / PORTRAIT_ATLAS_SIZE, 419 / PORTRAIT_ATLAS_SIZE, 512 / PORTRAIT_ATLAS_SIZE)
            normal:SetAlpha(MouseIsOver(self) and 1 or 0.75)
        end
    end
...
function SoundQueueUI:InitPortrait()
    ...
    function self.frame.portrait:Configure(soundData)
        ...
        elseif ShouldShowBookFor(soundData) then
            ...
            if not self.model._initialized then
                ...
                self.model:HookScript("OnHide", function(model)
                    model = model or this
                    if not model then return end
                    model:ClearModel()
                    model.oldCreatureID = nil
                    model.animation = nil
                    model.animDuration = nil
                    model.animDelay = nil
                    model.animtimer = nil
                end)
                self.model:HookScript("OnUpdate", function(model, elapsed)
                    model = model or this
                    ...
                    if model.animation and not model.animDuration then
                        local fileID = model.GetModelFileID and model:GetModelFileID()
                        model.animDuration = Utils:GetModelAnimationDuration(fileID, model.animation)
                        if model.animDuration and model.animDuration == 0 then
                            model.animation = 0
                    ...
                    if model.animDelay and not Addon.db.char.IsPaused then
                        model.animDelay = model.animDelay - elapsed
                        if model.animDelay < 0 then
                            model.animDelay = nil
                        else
                            return
                        end
                    ...
                    if not WAIT_FOR_ANIMATION_FINISH_BEFORE_IDLE and model.animation and model.animation ~= 0 and not model.animDelay and isPaused then
                        model.animation = 0
                        model.animtimer = nil
                        if model.SetAnimation then
                            model:SetAnimation(model.animation)
                    ...
                    if model.animation and GetTime() - (model.animtimer or 0) >= (model.animDuration or 2) then
                        if isPaused or model.animation == 0 then
                            if WAIT_FOR_ANIMATION_FINISH_BEFORE_IDLE and model.animation ~= 0 and not model.animDelay then
                                model.animation = 0
                                model.animtimer = nil
                                if model.SetAnimation then
                                    model:SetAnimation(model.animation)
                            ...
                            model.animtimer = GetTime()
                            if model.SetAnimation then
                                model:SetAnimation(model.animation)
                            end
                        end
                    end
                    model.wasPaused = isPaused
                end)
            end
            ...
            -- 1.12 has no UnitGUID; soundData.id is used as a uniqueness check so the anim timer is not reset when the queue changes but the displayed model does not
            local creatureID = soundData.id

            if creatureID ~= self.model.oldCreatureID then
                if self.model.SetCreature then
                    self.model:SetCreature(creatureID)
                end
                if self.model.SetCustomCamera then
                    self.model:SetCustomCamera(0)
                end
                if self.model.SetModelScale then
    ...
    local pauseNormal = self.frame.portrait.pause:GetNormalTexture()
    if pauseNormal then
        pauseNormal:ClearAllPoints()
        pauseNormal:SetPoint("CENTER")
        pauseNormal:SetSize(32, 32)
    end
    self.frame.portrait.pause:SetPushedTexture([[Interface\AddOns\AI_VoiceOver\Textures\PortraitFrameAtlas]])
    local pausePushed = self.frame.portrait.pause:GetPushedTexture()
    if pausePushed then
        pausePushed:ClearAllPoints()
        pausePushed:SetPoint("CENTER")
        pausePushed:SetSize(28, 28)
    end
    function self.frame.portrait.pause:Update()
        ...
        if Addon.db.char.IsPaused then
            ...
            normal:SetTexCoord(93 / PORTRAIT_ATLAS_SIZE, 186 / PORTRAIT_ATLAS_SIZE, 419 / PORTRAIT_ATLAS_SIZE, 512 / PORTRAIT_ATLAS_SIZE)
            pushed:SetTexCoord(93 / PORTRAIT_ATLAS_SIZE, 186 / PORTRAIT_ATLAS_SIZE, 419 / PORTRAIT_ATLAS_SIZE, 512 / PORTRAIT_ATLAS_SIZE)
            normal:SetAlpha(MouseIsOver(self) and 1 or 0)
        end
    end
    ...
    self.frame.portrait.pause:HookScript("OnLeave", function(self)
        local normal = self:GetNormalTexture()
        if normal then
            normal:SetAlpha(Addon.db.char.IsPaused and 0.75 or 0)
        end
    end)
...
function SoundQueueUI:InitMover()
    ...
    local moverNormal = self.frame.mover:GetNormalTexture()
    if moverNormal then
        moverNormal:SetTexCoord(462 / PORTRAIT_ATLAS_SIZE, 512 / PORTRAIT_ATLAS_SIZE, 462 / PORTRAIT_ATLAS_SIZE, 512 / PORTRAIT_ATLAS_SIZE)
        moverNormal:ClearAllPoints()
        moverNormal:SetPoint("CENTER")
        moverNormal:SetSize(16, 16)
    end
    self.frame.mover:SetPushedTexture([[Interface\AddOns\AI_VoiceOver\Textures\PortraitFrameAtlas]])
    local moverPushed = self.frame.mover:GetPushedTexture()
    if moverPushed then
        moverPushed:SetTexCoord(462 / PORTRAIT_ATLAS_SIZE, 512 / PORTRAIT_ATLAS_SIZE, 462 / PORTRAIT_ATLAS_SIZE, 512 / PORTRAIT_ATLAS_SIZE)
        moverPushed:ClearAllPoints()
        moverPushed:SetPoint("CENTER")
        moverPushed:SetSize(14, 14)
    end
