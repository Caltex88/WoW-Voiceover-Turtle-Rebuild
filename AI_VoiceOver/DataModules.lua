...
-- Temporarily enable "Load out of date AddOns" so data modules with a mismatched ##Interface still load, then restore the user's setting.
...
function DataModules:LoadModule(module)
    ...
    if not loaded and DEFAULT_CHAT_FRAME then
        DEFAULT_CHAT_FRAME:AddMessage("|cffff5555VoiceOver: failed to load " .. tostring(module.AddonName) .. " (" .. tostring(reason) .. ")|r")
    end
...
function DataModules:GetQuestID(source, title, npcName, text)
    ...
    for _, module in self:GetModules() do
        ...
        local sourceLookup = data and source and data[source]
            ...
            if titleLookup then
                if type(titleLookup) == "number" then
                    ...
                    if not npcLookup then
                        -- Turtle NPC names can differ; if the title is unique, use that ID.
                        local uniqueID, uniqueCount
                        uniqueCount = 0
                        for _, value in pairs(titleLookup) do
